...
public:
	...
	void printCharacter() // Prints name and funds
	{
		cout << " Name:" << name << "\n Money:" << money << endl;
		printBackpack();
	}
	...
	void printBackpack() 
	{
		cout << " " << name << ", you own " << numItems << " Weapons:" << endl; // Prints owned weapons 
...
{
	string pname;
	BinaryTree *ht = new BinaryTree(); // Create pointer to bin tree
	cout << "Please enter Player name: " << endl;
	cin >> pname;
	Player pl(pname, 100); // create player object passing name and money valuer as parameters
	addWeapons(ht); 
	showRoom(ht, &pl); // Showroom takes the inventory binary tree and player params
	pl.printCharacter(); // Prints character info with weapons
